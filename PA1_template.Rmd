...
The histogram shows a single main peak centered at 10000 steps. The mean number of steps taken per day is `r meanSteps` and the median number of steps is `r medianSteps`, consistent with the histogram.
...
The most activity, averaged across all days, is observed for the interval `r maxInterval` (presumably while the subject is commuting to work/school, either on foot or by public transportation). Activity is highest in the daytime hours, tapering off towards evening and virtually ceasing at night.
...
Using this strategy, we have filled in all `r missing` missing values and determined a new mean of `r imputedMeanSteps` steps and median `r imputedMedianSteps`. This is very slightly higher than our previous mean and median, `r meanSteps` and `r medianSteps` respectively, but otherwise the two histograms show a very similar pattern.
